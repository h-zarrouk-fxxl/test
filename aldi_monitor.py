...
"""
╔══════════════════════════════════════════════════════════════════════════╗
║          ALDI Talk Auto-Refill Monitor  v8  (Termux / Android)          ║
║                                                                          ║
...
# Sucht und klickt Bestätigungs-Buttons in Shadow DOM (Modal nach erstem Klick)
_JS_CLICK_CONFIRM = r"""
(function() {
  // Texte, die auf Bestätigungsbutton hindeuten
  var confirmPhrases = [
    'jetzt buchen', 'buchen', 'bestätigen', 'bestätigung', 'confirm',
    'ja', 'yes', 'ok', 'weiter', 'abschließen', 'aktivieren',
    'kaufen', 'kostenpflichtig', 'akzeptieren'
  ];
  // Texte, die auf Abbrechen/Schließen hindeuten (bevorzuge Bestätigung)
  var cancelPhrases = ['abbrechen', 'cancel', 'nein', 'no', 'schließen', 'close'];

  var candidates = [];

  function score(text) {
    var t = text.toLowerCase().trim();
    for (var i = 0; i < cancelPhrases.length; i++) {
      if (t === cancelPhrases[i] || t.indexOf(cancelPhrases[i]) >= 0) return -1;
    }
    for (var i = 0; i < confirmPhrases.length; i++) {
      if (t === confirmPhrases[i]) return 10 - i;   // Exakt-Match bevorzugt
      if (t.indexOf(confirmPhrases[i]) >= 0) return 5 - i;
    }
    return 0;
  }

  function collectConfirm(root, depth) {
    if (depth > 15 || !root) return;
    // Suche in Modals/Dialogen zuerst
    var selectors = [
      '[role="dialog"] one-button', '[role="dialog"] button',
      '[class*="modal"] one-button', '[class*="modal"] button',
      '[class*="overlay"] one-button', '[class*="overlay"] button',
      'one-dialog one-button', 'one-dialog button',
      'one-modal one-button', 'one-modal button',
    ];
    for (var s = 0; s < selectors.length; s++) {
      try {
        ...
        for (var i = 0; i < els.length; i++) {
          ...
          var sc = score(t);
          if (sc > 0) candidates.push({el: els[i], text: t, score: sc, depth: depth});
        }
      } catch(e) {}
    }
    ...
    var all = root.querySelectorAll ? root.querySelectorAll('*') : [];
    for (var i = 0; i < all.length; i++) {
      if (all[i].shadowRoot) collectConfirm(all[i].shadowRoot, depth + 1);
    }
  }

  collectConfirm(document, 0);

  if (candidates.length === 0) {
    return {clicked: false, reason: 'no_confirm_dialog'};
  }

  // Nimm den besten Kandidaten
  candidates.sort(function(a, b) { return b.score - a.score || a.depth - b.depth; });
  var best = candidates[0];
  var target = best.el;

  // Prüfe disabled
  if (target.hasAttribute && target.hasAttribute('disabled')) {
    return {clicked: false, reason: 'disabled', text: best.text};
  }

  var method = 'outer';
  // Wenn one-button: klicke inneren shadow button
  if (target.tagName && target.tagName.toLowerCase() === 'one-button' && target.shadowRoot) {
    var inner = target.shadowRoot.querySelector('button');
    if (inner && !inner.hasAttribute('disabled')) {
      inner.scrollIntoView({block:'center'});
      inner.focus();
      inner.click();
      inner.dispatchEvent(new MouseEvent('click',{bubbles:true,cancelable:true,composed:true}));
  ...
  return {clicked: true, method: method, text: best.text, score: best.score, total: candidates.length};
})()
"""
...
async def attempt_booking(
    ...
    try:
        ...
        # Methode B: JS Shadow-DOM — IMMER ausführen (Playwright-Klick triggert Vue nicht)
        await page.wait_for_timeout(300)  # Kurz warten nach Playwright-Klick
        result_js = await page.evaluate(_JS_CLICK_BUTTON)
        if result_js and result_js.get("clicked"):
            clicked = True
            log.info("   🖱️  JS-Klick (Shadow-DOM %s, Text: '%s')",
                     result_js.get("method"), result_js.get("text"))
        elif result_js and not clicked:
            reason = result_js.get("reason", "?")
            log.error("   ❌ Kein Button (JS): %s", reason)
            if reason == "disabled":
                log.warning("   Button gefunden aber disabled. Buchung aktuell nicht möglich.")
                page.remove_listener("response", on_response)
                return False, None
            else:
                log.warning("   Alle Buttons auf Seite: %s", result_js.get("all_buttons", []))
        ...
        # ── Schritt 1b: Bestätigungs-Dialog (falls vorhanden) ────────────────
        # ALDI Talk zeigt ggf. eine Confirmation Modal — auch im Shadow DOM
        await page.wait_for_timeout(2000)
        confirm_result = await page.evaluate(_JS_CLICK_CONFIRM)
        if confirm_result and confirm_result.get("clicked"):
            log.info("   ✅ Bestätigungs-Dialog geklickt: '%s' [%s]",
                     confirm_result.get("text"), confirm_result.get("method"))
            await page.wait_for_timeout(1500)
        else:
            log.debug("   (Kein Bestätigungs-Dialog gefunden — %s)",
                      confirm_result.get("reason") if confirm_result else "keine Antwort")
        ...
        # Noch einmal Bestätigungs-Dialoge suchen (können verzögert erscheinen)
        await page.wait_for_timeout(1000)
        confirm_result2 = await page.evaluate(_JS_CLICK_CONFIRM)
        if confirm_result2 and confirm_result2.get("clicked"):
            log.info("   ✅ Nachgelagerter Dialog geklickt: '%s'", confirm_result2.get("text"))
            await page.wait_for_timeout(1500)
...
async def main():
    ...
    parser = argparse.ArgumentParser(
        description="ALDI Talk Auto-Refill Monitor v8",
    ...
    log.info("╔════════════════════════════════════════╗")
    log.info("║  ALDI Talk Auto-Refill Monitor  v8     ║")
    log.info("╚════════════════════════════════════════╝")
