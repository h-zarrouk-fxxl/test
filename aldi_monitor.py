...
async def do_login(page: Page, username: str, password: str) -> bool:
    ...
    log.info("🔐 Navigiere zur Login-Seite...")
    try:
        await page.goto(LOGIN_URL, wait_until="domcontentloaded", timeout=30000)
    except Exception as e:
        log.error(f"Login-Navigation fehlgeschlagen: {e}")
        return False
    ...
    # ── Username ─────────────────────────────────────────────────────────────
    # ForgeRock/OpenAM Login (login.alditalk-kundenbetreuung.de/signin/XUI/)
    # nutzt idToken1/idToken2 Felder
    user_selectors = [
        '#idToken1',                    # ForgeRock/OpenAM (ALDI Talk nutzt das!)
        'input[name="IDToken1"]',
        'input[type="tel"]',
        'input[name="username"]',
        'input[name="phone"]',
        'input[id*="user"]',
        'input[placeholder*="Rufnummer"]',
    ...
    if not user_field:
        log.error("❌ Kein Username-Eingabefeld gefunden!")
    ...
    # ── Passwort ──────────────────────────────────────────────────────────────
    pw_field = None
    for sel in ['#idToken2', 'input[name="IDToken2"]', 'input[type="password"]', 'input[name="password"]', 'input[id*="pass"]']:
        try:
            f = await page.query_selector(sel)
            if f and await f.is_visible():
                pw_field = f
    ...
    submitted = False
    for sel in [
        '#loginButton_0',               # ForgeRock Submit
        'input[type="submit"]',
        'button[type="submit"]',
    ...
    log.info("   Login abgeschickt – warte auf Weiterleitung...")
    ...
    try:
        ...
        log.warning("   Weiterleitung hat zu lange gedauert")
